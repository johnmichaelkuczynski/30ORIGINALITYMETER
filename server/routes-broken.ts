...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // NEW 160-METRIC COMPREHENSIVE ANALYSIS ENDPOINT
  app.post("/api/analyze/comprehensive", async (req, res) => {
    try {
      ...
      // Import and use the comprehensive analysis system
      const { performComprehensiveAnalysis } = await import("./lib/comprehensiveAnalysis");
      
      // Perform the analysis with the specified provider
  ...
  app.post("/api/analyze/single", async (req, res) => {
    try {
      ...
      // REMOVE ALL HARDCODED LOGIC - PURE PASSTHROUGH TO LLM
      console.log(`Pure passthrough analysis using ${provider} - no filtering or hardcoding`);
