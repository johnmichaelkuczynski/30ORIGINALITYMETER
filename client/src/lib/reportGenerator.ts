  ...
  try {
    ...
    if (result.conceptualOverlap) {
      ...
      if (isSinglePassageMode && overlapData.passageA) {
        const text = `Score: ${overlapData.passageA.score}/10 - ${overlapData.passageA.label}`;
        pdf.text(text, 20, y);
        y += 5;
      ...
      } else if (!isSinglePassageMode && overlapData.score !== undefined) {
        const text = `Score: ${overlapData.score}/10 - ${overlapData.label}`;
        pdf.text(text, 20, y);
        y += 5;
    ...
    if (result.novelty) {
      ...
      if (isSinglePassageMode && noveltyData.passageA) {
        const text = `Score: ${noveltyData.passageA.score}/10 - ${noveltyData.passageA.label}`;
        pdf.text(text, 20, y);
        y += 5;
      ...
      } else if (!isSinglePassageMode && noveltyData.score !== undefined) {
        const text = `Score: ${noveltyData.score}/10 - ${noveltyData.label}`;
        pdf.text(text, 20, y);
        y += 5;
...
function generateSummary(
  ...
  if (isSinglePassageMode) {
    ...
    const originalityScore = result.novelty?.passageA?.score || 0;
    ...
    const overlapScore = result.conceptualOverlap?.score || 0;
    ...
    if (result.novelty?.passageA?.score && result.novelty?.passageB?.score) {
      const noveltyA = result.novelty.passageA.score;
      const noveltyB = result.novelty.passageB.score;
...
function extractScores(
  ...
  if (isSinglePassageMode) {
    ...
    if (result.novelty?.passageA) {
      scores.originality = {
        label: "Originality",
        score: result.novelty.passageA.score || 0,
    ...
    if (result.conceptualLineage?.passageA) {
      ...
      scores.framework = {
        ...
        score: calculateFrameworkScore(result.conceptualLineage.passageA),
    ...
    if (result.coherence?.passageA) {
      scores.coherence = {
        label: "Coherence",
        score: result.coherence.passageA.score || 0,
    ...
    if (result.parasiteIndex?.passageA) {
      scores.parasitism = {
        ...
        score: convertParasiteLevelToScore(result.parasiteIndex.passageA.level),
    ...
    if (result.aiDetection?.passageA) {
      scores.aiDetection = {
        label: "AI Detection",
        score: result.aiDetection.passageA.isAIGenerated ? 2 : 8, // Lower score if AI-generated
    ...
    if (result.conceptualOverlap) {
      scores.overlap = {
        ...
        score: result.conceptualOverlap.score || 0,
    ...
    if (result.novelty?.passageA && result.novelty?.passageB) {
      ...
      scores.originalityB = {
        ...
        score: result.novelty.passageB.score || 0,
    ...
    if (result.aiDetection?.passageA && result.aiDetection?.passageB) {
      ...
      scores.aiDetectionA = {
        ...
        score: result.aiDetection.passageA.isAIGenerated ? 2 : 8,
        justification: result.aiDetection.passageA.details || 
          `The text appears to be ${result.aiDetection.passageA.isAIGenerated ? "AI-generated" : "human-written"} with ${result.aiDetection.passageA.confidence} confidence.`,
        quotes: []
      };
      ...
      scores.aiDetectionB = {
        ...
        score: result.aiDetection.passageB.isAIGenerated ? 2 : 8,
...
function extractStrengths(
  ...
  if (isSinglePassageMode) {
    ...
    if (result.novelty?.passageA?.score && result.novelty.passageA.score >= 7) {
      strengths.push("Demonstrates high originality and innovative thinking.");
    }
    
    if (result.coherence?.passageA?.score && result.coherence.passageA.score >= 7) {
    ...
    if (result.conceptualOverlap?.score && result.conceptualOverlap.score >= 7) {
    ...
    if (result.novelty?.passageB?.score && result.novelty.passageB.score >= 7) {
...
function extractWeaknesses(
  ...
  if (isSinglePassageMode) {
    ...
    if (result.novelty?.passageA?.score && result.novelty.passageA.score <= 4) {
      weaknesses.push("Lacks originality; relies heavily on established ideas without significant innovation.");
    }
    
    if (result.coherence?.passageA?.score && result.coherence.passageA.score <= 4) {
    ...
    if (weaknesses.length < 2) {
      if (!result.novelty?.passageA || (result.novelty.passageA.score && result.novelty.passageA.score < 7)) {
    ...
    if (result.conceptualOverlap?.score && result.conceptualOverlap.score <= 4) {
    ...
    if (result.novelty?.passageB?.score && result.novelty.passageB.score <= 4) {
      const title = result.novelty.passageB.title || "Document B";
      weaknesses.push(`"${title}" lacks originality and relies heavily on established ideas.`);
    }
...
function generateImprovementSuggestions(
  ...
  if (isSinglePassageMode) {
    ...
    if (result.novelty?.passageA?.score !== undefined) {
      if (result.novelty.passageA.score <= 4) {
      ...
      } else if (result.novelty.passageA.score <= 6) {
    ...
    if (result.conceptualOverlap?.score !== undefined && result.conceptualOverlap.score <= 5) {
    ...
    if (result.novelty?.passageA?.score !== undefined && result.novelty.passageA.score <= 5) {
    ...
    if (result.novelty?.passageB?.score !== undefined && result.novelty.passageB.score <= 5) {
...
function generateLiteratureContext(
  ...
  if (isSinglePassageMode && result.conceptualLineage?.passageA) {
    ...
    if (result.novelty?.passageA?.score !== undefined) {
      if (result.novelty.passageA.score >= 8) {
    ...
    context += "Together, they " + 
      (result.conceptualOverlap.score !== undefined && result.conceptualOverlap.score >= 6 
